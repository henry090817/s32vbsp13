...
int fsl_gpios[] = {18,16,17};

#define ENTER \
do{ if(debug_level >= 1) printk(KERN_INFO "[FSL_GPIO_KEYS_DBG] func: %s  line: %04d\n", __func__, __LINE__); }while(0)

#define PRINT_DBG(format,x...)  \
do{ if(debug_level >= 1) printk(KERN_INFO "[FSL_GPIO_KEYS_DBG] " format, ## x); }while(0)

#define PRINT_INFO(format,x...)  \
do{ printk(KERN_INFO "[FSL_GPIO_KEYS_INFO] " format, ## x); }while(0)

#define PRINT_WARN(format,x...)  \
do{ printk(KERN_INFO "[SPRD_GPIO_KEYS_WARN] " format, ## x); }while(0)

#define PRINT_ERR(format,x...)  \
do{ printk(KERN_ERR "[FSL_GPIO_KEYS_ERR] func: %s  line: %04d  info: " format, __func__, __LINE__, ## x); }while(0)
